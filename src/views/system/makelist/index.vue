              ...
              <el-row :gutter="0">
                ...
                <el-col :span="8">
                  <el-form-item label="单据状态" prop="status">
                    <el-input v-model="form.statusName" placeholder="自动生成" :disabled="true" />
                  </el-form-item>
                </el-col>
                <el-col :span="8">
...
import {
  ...
  delProjectInventory,
  exportProjectInventory,
  getProjectInventoryList
} from "@/api/system/projectInventory";
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        djTitle: [{ required: true, message: "标题不能为空", trigger: "blur" }],
    ...
    changemoney() {
      if (this.form1.inventoryNum != "" && this.form1.inventoryPrice != "") {
        this.form1.inventoryMoney =
          Number(this.form1.inventoryNum) * Number(this.form1.inventoryPrice);
      }
    },
    /** 查询部门下拉树结构 */
    getTreeselect() {
          ...
          listProjectInvents(data.projectCode).then(response => {
            ...
            if (response.rows.length > 0) {
              ...
              getProjectInventoryList(this.form.djNumber).then(response => {
                ...
                for (var i = 0; i < response.data.length; i++) {
                  response.data[i].parentCode = Number(
                    response.data[i].parentCode
                  );
                  if (response.data[i].isLast == 0) {
                    response.data[i].isLast = "无下级";
                  } else {
