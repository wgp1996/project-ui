    ...
    <el-dialog :title="title" :visible.sync="open" width="700px">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="父级编号" prop="feeTypePid">
          <treeselect
            v-model="form.feeTypePid"
            :options="feeTypeOptions"
            :normalizer="normalizer"
            placeholder="请选择费用分类父id"
          />
        </el-form-item>
        <el-form-item label="费用分类名称" prop="feeTypeName">
          <el-input
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        feeTypeName: [
          { required: true, message: "费用分类名称不能为空", trigger: "blur" },
        ],
      },
    };
  },
  ...
  methods: {
    ...
    getList() {
      ...
      listFeeType(this.queryParams).then((response) => {
        ...
        this.feeTypeList = this.handleTree(
          response.data,
          "feeTypeId",
          "feeTypePid"
        );
    ...
    handleDelete(row) {
      this.$confirm(
        '是否确认删除费用项目分类编号为"' + row.feeTypeId + '"的数据项?',
        "警告",
        {
