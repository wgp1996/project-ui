    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleEffect"
          v-hasPermi="['system:purchaseSettlement:examine']"
          >审核</el-button
        >
      </el-col>
      <el-col :span="1.5" v-if="showCancel">
        <el-button
          ...
          :disabled="multiple"
          @click="handleCancel"
          v-hasPermi="['system:purchaseSettlement:cancelAudit']"
          >取消审核</el-button
        >
      </el-col>
    </el-row>
    ...
    <el-dialog title="流程审核" :visible.sync="openLcsh" width="500px">
      <el-form ref="shForm" :model="shForm" :rules="rules" label-width="120px">
        <el-form-item label="审核结果" prop="status">
          <el-radio-group v-model="shForm.status" @change="changeStatus">
        ...
        <el-form-item label="批复金额" prop="pfMoney" v-if="isShow">
          <el-input
            v-model="shForm.pfMoney"
      ...
      <el-tabs v-model="activeName">
        <el-tab-pane label="基本信息" name="first">
          ...
          >
            ...
            <el-form-item label="批复结算金额" prop="pfPayMoney">
              <el-input v-model="form.pfPayMoney" placeholder="批复结算金额" />
            </el-form-item>
        ...
        <el-tab-pane label="结算明细" name="second">
          ...
          >
            ...
            <el-table-column prop="orderDjNumber" label="入库单号" width="130">
              <template scope="scope">
                <el-input
                  :disabled="true"
                  size="small"
            ...
            <el-table-column prop="goodsCode" label="物资编码" width="130">
              <template scope="scope">
                <el-input
                  :disabled="true"
                  size="small"
            ...
            <el-table-column prop="goodsName" label="物资名称" width="130">
              <template scope="scope">
                <el-input
                  :disabled="true"
                  size="small"
            ...
            <el-table-column prop="goodsNum" label="订单数量" width="130">
              <template scope="scope">
                <el-input
                  size="small"
            ...
            <el-table-column prop="goodsPrice" label="单价" width="130">
              <template scope="scope">
                <el-input
                  size="small"
                  :readonly="true"
...
<script>
import {
  updatePurchaseSettlementBySh,
  shListPurchaseSettlement,
  getPurchaseSettlement,
  examinePurchaseSettlement,
  cancelAuditPurchaseSettlement,
} from "@/api/system/purchaseSettlement";
import { getInfo } from "@/api/login";
import supplierSelect from "./supplierSelect";
import { systemFileList } from "@/api/system/projectInfo";
import { djFlowList } from "@/api/system/flowInfo";
import { listPurchaseSettlementChild } from "@/api/system/purchaseSettlementChild";
export default {
  ...
  data() {
    return {
      ...
      spRules: {
        pfPayMoney: [
          { required: true, message: "批复金额不能为空!", trigger: "change" },
        ],
      },
    };
  },
  ...
  methods: {
    changeStatus(data) {
      ...
      if(data==1){
        ...
        this.isShow=false;
        this.shForm.pfMoney="-1";
        //取消批复金额必须校验
         //this.$refs['shForm'].resetFields(['pfMoney']);
      }
    },
    /** 选择供货商 */
    supplierSelect() {
    ...
    getList() {
      this.loading = true;
      shListPurchaseSettlement(this.queryParams).then((response) => {
    ...
    handleSelectionChange(selection) {
      this.djNums = selection.map((item) => item.djNumber);
    ...
    handleEffect(row) {
      ...
      this.shForm.djId = this.djNums[0];
    ...
    /** 取消按钮操作 */
    handleCancel(row) {
      const djNums = row.id || this.djNums;
      ...
      this.$confirm("是否确认取消选择的数据项?", "警告", {
        ...
        .then(function () {
          return cancelAuditPurchaseSettlement(djNums, nodeNos);
        })
        .then(() => {
          this.getList();
    ...
    submitForm: function () {
      ...
      this.$refs["shForm"].validate((valid) => {
        if (valid) {
          //通过
          if(this.shForm.status==1){
              let form = {
                pfPayMoney: this.shForm.pfMoney,
                id: this.ids[0],
              };
              //修改批复金额
              updatePurchaseSettlementBySh(form).then((response) => {
          ...
          examinePurchaseSettlement(this.shForm).then((response) => {
            if (response.code === 200) {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != undefined) {
            updatePurchaseSettlementBySh(this.form).then((response) => {
              if (response.code === 200) {
